// 重新構建知識庫並向量化 - 直接從數據庫讀取 WhatsApp 數據
...
const SESSION_ID = 'sess_id73sa6oi_1770363274857'; // 使用當前活躍的 session
...
async function main() {
    ...
    console.log('🚀 WhatsApp 知識庫重建與向量化');
    console.log('='.repeat(60));
    ...
    try {
        ...
        if (documents.length === 0) {
            console.log('⚠️  沒有找到任何數據，請檢查 Session ID 是否正確');
            return;
        }
        ...
        console.log(`📊 成功率: ${((successCount / documents.length) * 100).toFixed(1)}%`);
        console.log('='.repeat(60));
